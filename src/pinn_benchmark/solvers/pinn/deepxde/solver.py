...
def solve_deepxde(problem: Problem, neurons_per_layer=10, quadrature_points=10):
    ...
    n = quadrature_points
    data = dde.data.PDE(geom, problem.pde, [], num_domain=n**2, num_boundary=4*n, num_test=(2*n)**2)

    net = dde.maps.FNN([problem.domain.dim] + [neurons_per_layer] * num_hidden_layers + [1], "tanh", "Glorot uniform")
