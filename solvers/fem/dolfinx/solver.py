...
def solve_dolfinx(problem: Problem, resolution=16):
    ...
    comm = MPI.COMM_WORLD
    msh = mesh.create_rectangle(comm=comm,
    ...
    facets = mesh.locate_entities_boundary(msh, dim=dim-1, marker=lambda x: bc.boundary(x))
    ...
    lin_prob = fem.petsc.LinearProblem(a, b, bcs=[bc], petsc_options={"ksp_type": "preonly", "pc_type": "lu"})
